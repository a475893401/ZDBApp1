...
public class SaleTopFragment extends BaseHttpFragment<OrderTopD> implements ViewPager.OnPageChangeListener {

    private CustomPagerSlidingTabStrip topTab;
    private MyViewPager myViewPager;
    private TopFragmentAdapter topFragmentAdapter;

    private String[] tabTitle = {"我收到的", "我申请的"};
    private static final int VIEW_1 = 0;
    private static final int VIEW_2 = 1;

    private static int VIEW_SIZE = 2;

    private MyGetSaleFragment gradOrderFragment = null;
    private MyApplyFragment takingOrderFragment = null;

    public static SaleTopFragment getInstance() {
                ...
                switch (position) {
                    case VIEW_1:
                        if (null == gradOrderFragment)
                            gradOrderFragment = MyGetSaleFragment.getInstance();
                        return gradOrderFragment;
                    case VIEW_2:
                        if (null == takingOrderFragment)
                            takingOrderFragment = MyApplyFragment.getInstance();
