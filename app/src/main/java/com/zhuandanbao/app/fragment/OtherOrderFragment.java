...
public class OtherOrderFragment extends BaseListViewFragment<OrderListTopD> implements ViewPager.OnPageChangeListener {
    ...
    private String[] tabTitle = {"待处理", "处理中", "已完成"};
    ...
    private OtherOrderListFragment fragment1 = null, fragment2 = null, fragment3 = null;
        ...
        public Fragment getItem(int position) {
            if (position >= 0 && position < VIEW_SIZE) {
                switch (position) {
                    case VIEW_1:
                        if (null == fragment1)
                            fragment1 = OtherOrderListFragment.getInstance(VIEW_1);
                        return fragment1;
                    case VIEW_2:
                        if (null == fragment2)
                            fragment2 = OtherOrderListFragment.getInstance(VIEW_2);
                        return fragment2;
                    case VIEW_3:
                        if (null == fragment3)
                            fragment3 = OtherOrderListFragment.getInstance(VIEW_3);
                        return fragment3;
                }
            }
            return null;
