 ...
 * @author jing.huai
 */
public class ImChattingOperationCustom extends IMChattingPageOperateion {
    ...
    // 默认写法
    public ImChattingOperationCustom(Pointcut pointcut) {
        super(pointcut);
    }
    /**
    ...
    public static void sendGeoMessage(YWConversation conversation) {
        ShopLatEntity shopLatEntity= (ShopLatEntity) MainApplication.cache.getAsObject(Constants.SHOP_LAT_INFO);
        conversation.getMessageSender().sendMessage(YWMessageChannel.createGeoMessage(shopLatEntity.lat,
                shopLatEntity.lon, shopLatEntity.address), 120, null);
    }
...
//            return true;
            ...
            MLog.e( "你点击了地理位置消息"+message.getAuthorUserId()+"toString="+message.toString());
...
//                IMNotificationUtils.getInstance().showToast(fragment.getActivity(), "你点击了自定义消息");
    ...
    public boolean onMessageLongClick(final Fragment fragment, final YWMessage message) {
        ...
        MLog.e( "触发了自定义MessageLongClick事件");
...
//                    linkedList.add(0,context.getString(R.string.demo_forward));
            ...
            final YWConversation conversation =ImLoginHelper.getInstance().getIMKit().getConversationService().getConversationByConversationId(message.getConversationId());
                    ...
                    .setItems(strs, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog,
                            ...
                            if (which < strs.length) {
                                if (context.getString(R.string.demo_forward).equals(strs[which])) {
//                                    showForwardMessageDialog(context, message);//轉發
...
//            IMNotificationUtils.getInstance().showToast(context, "你长按了自定义消息");
    ...
    public View getCustomUrlView(Fragment fragment, YWMessage message, String url, YWConversation ywConversation) {
        if (url.equals("https://www.baidu.com/ ")) {
            LinearLayout layout = (LinearLayout) View.inflate(MainApplication.getInstance(), R.layout.demo_custom_tribe_msg_layout, null);
...
//            if(isConversationFirstCreated){
                final SharedPreferences defalultSprefs = fragment.getActivity().getSharedPreferences("ywPrefsTools", Context.MODE_PRIVATE);
...
//        else {
//            YWMessage message = YWMessageChannel.createTextMessage("hi，我是单聊自定义消息之好友名片");
//            return message;
//        }
        //返回null,则不发送
        return null;
    ...
    public List<ReplyBarItem> getCustomReplyBarItemList(final Fragment pointcut, final YWConversation conversation, List<ReplyBarItem> replyBarItemList) {
        ...
        for (ReplyBarItem replyBarItem : replyBarItemList) {
            if(replyBarItem.getItemId()== YWChattingPlugin.ReplyBarItem.ID_ALBUM){
                //是否隐藏ReplyBarItem中的选择照片选项ID_ALBUM
