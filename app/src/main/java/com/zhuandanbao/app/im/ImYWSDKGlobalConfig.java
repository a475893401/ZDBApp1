 ...
 * @author shuheng
 */
public class ImYWSDKGlobalConfig extends YWSDKGlobalConfig {


    private ImYWSDKGlobalConfig(Pointcut pointcut) {
        super(pointcut);
    }

    //用于给主程序层拿配置
    public volatile static ImYWSDKGlobalConfig mInstance =null;

    public static ImYWSDKGlobalConfig getInstance(){
        if (mInstance == null) {
            synchronized(ImYWSDKGlobalConfig.class) {
                if (mInstance == null) {
                    mInstance = new ImYWSDKGlobalConfig(null);
                }
            }
        }
        return mInstance;
